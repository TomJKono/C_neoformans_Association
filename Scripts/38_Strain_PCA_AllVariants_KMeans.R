...
# clusters of data is statistically sound. This version of the script uses
# the PCA from all variants.
...
pdf(file="PC_Euclidean_Distances_AllVariants.pdf", height=8, width=8)
fviz_dist(euc_dist)
dev.off()
...
pdf(file="Gap_Statistic_AllVariants.pdf", height=6, width=6)
...
pdf(file="Silhouette_Scores_AllVariants.pdf", height=6, width=6)
plot(
    s_dists ~ k_vals,
    xlim=c(1, 10),
    ylim=c(-1, 1),
...
fin_clust <- kmeans(clust_in, centers=2, nstart=50, iter.max=1000)
clustered <- clust_in
clustered$IID <- rownames(clust_in)
clustered$Cluster <- factor(fin_clust$cluster)
pvar <- eigval$V1 / sum(eigval$V1)
pdf(file="KMeans_PCA_Labeled_AllVariants.pdf", height=6, width=6)
