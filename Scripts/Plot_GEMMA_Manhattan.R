...
setwd("/Users/tomkono/Dropbox/GitHub/RIS/Nielsen_Cn_GWAS/Results/38_Strain_GWAS/ST93A_Isolates/Manhattan_Plots/GEMMA")
...
# Define a lil function to draw a plot
manhattan_plot <- function(assoc_file) {
    gwas_res <- read.table(assoc_file, header=TRUE)
    pheno_name <- gsub(".assoc.txt", "", basename(assoc_file))
    chrom <- gwas_res$chr
    pos <- gwas_res$ps
    pval <- gwas_res$p_lrt
...
assoc_dir <- "/Users/tomkono/Dropbox/GitHub/RIS/Nielsen_Cn_GWAS/Results/38_Strain_GWAS/Associations/output"
#   For the ST93A subset
# assoc_dir <- "/Users/tomkono/Dropbox/GitHub/RIS/Nielsen_Cn_GWAS/Results/38_Strain_GWAS/ST93A_Isolates/Associations/GEMMA/output"
